...
private:
    ...
    void fake_lookup_table(const uint32_t len, const uint32_t input_bw, const uint32_t output_bw,
...
public:
    ...
    ~SPLUT_protocol();
    void offline_LUT();
    void mill_and_equal_test_1_bit(const uint32_t len, const uint8_t * x, uint8_t * mill_res, uint8_t * equal_test_res);
    void mill_and_equal_test(uint32_t len, uint8_t * x, uint8_t * mill_res, uint8_t * equal_test_res, const uint32_t bw);
